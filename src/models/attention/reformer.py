# https://github.com/HazyResearch/fly/blob/master/src/models/attention/reformer_attention.py
...
from ..model_utils.hash_utils import batched_index_select, invert_permutation, pad_to_multiple, sort_key_val
from ..model_utils.mask_utils import FullMask, LengthMask, pad_mask
...
class ReformerAttention(nn.Module):
    ...
    def forward(self, qk, key, value, **kwargs):
        key_padding_mask = kwargs["key_padding_mask"]
        qk = rearrange(qk, "b n (h d) -> b n h d", h=self.num_heads, d=self.dim_per_head)
        ...
        value = pad_to_multiple(value, self.bucket_size * 2, dims=1)
        ...
        if S > seqlen_og:
            if key_padding_mask is None:
                key_padding_mask = LengthMask(qk.new_full((qk.shape[0],), seqlen_og, dtype=torch.long), max_len=S)
            else:
                key_padding_mask = pad_mask(key_padding_mask, pad_length=S - seqlen_og, left=False, value=False)
        if key_padding_mask is not None and not key_padding_mask.all_ones:
            # Repeat for all heads and all hash functions
            key_padding_mask_bool = repeat(key_padding_mask.bool_matrix, "b s -> (b head) s", head=H)
        else:
        ...
        perm_oneback = look_one_back(
            rearrange(perm, "h b (nbuckets bucketsz) -> (h b) nbuckets bucketsz", nbuckets=n_buckets)
        )
        perm_oneback = rearrange(perm_oneback, "(h b) nbuckets2 bucketsz -> h b (nbuckets2 bucketsz)", h=self.n_hashes)
        ...
        tv = sort_to_buckets(value, perm_oneback, self.bucket_size * 2)

        # Dot-product attention.
        inner = torch.einsum("zbhie,zbhje->zbhij", tq, tk) * softmax_temp
        masked_value = max_neg_value(inner)

        bq_idx = rearrange(perm, "h b (nbuckets bucketsz) -> h b nbuckets bucketsz 1", bucketsz=self.bucket_size)
        bkv_idx = rearrange(
            perm_oneback, "h b (nbuckets bucketsz2) -> h b nbuckets 1 bucketsz2", bucketsz2=self.bucket_size * 2
        )

        # mask out attention to padded tokens
        if key_padding_mask is not None and not key_padding_mask.all_ones:
            s_key_padding_mask = sort_to_buckets(
                rearrange(key_padding_mask_bool, "b s -> b s 1"), perm_oneback, self.bucket_size * 2
            )
        ...
        if not self._attend_across_buckets:
            bq_buckets = sort_to_buckets(
                rearrange(buckets, "h b s -> h b s 1"), perm, self.bucket_size, unsqueeze=False
            )
            bkv_buckets = sort_to_buckets(
                rearrange(buckets, "h b s -> h b s 1"), perm_oneback, self.bucket_size * 2, unsqueeze=False
            )
        ...
        if not self._allow_duplicate_attention:
            ...
            bq_locs = repeat(slocs[..., :total_hashes], "h b nbuckets bucketsz nh -> h b nbuckets bucketsz 1 (2 nh)")
            bkv_locs = look_one_back(rearrange(slocs, "h b nbuckets bucketsz nh2" "-> (h b) nbuckets bucketsz nh2"))
            bkv_locs = rearrange(
                bkv_locs, "(h b) nbuckets bucketsz2 nh2 -> h b nbuckets 1 bucketsz2 nh2", h=self.n_hashes
            )
